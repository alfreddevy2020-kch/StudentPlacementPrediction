...
logreg_configs = {
    "Extreme Regularization\n(C=0.0001)": {
        ...
        "model": LogisticRegression(
            C=0.0001, penalty="l2", solver="lbfgs", max_iter=2000,
    ...
    "Weak Regularization\n(C=100)": {
        ...
        "color": COLORS["good"],
        "model": LogisticRegression(
            C=100, penalty="l2", solver="lbfgs", max_iter=2000,
            class_weight=class_weights, random_state=42,
        ),
    ...
    "No Regularization\n(C=100000)": {
        ...
        "model": LogisticRegression(
            C=100000, penalty="l2", solver="lbfgs", max_iter=5000,
            class_weight=class_weights, random_state=42,
        ),
...
for name, cfg in logreg_configs.items():
    ...
    print(f"    CV F1={metrics['CV F1']}, Test F1={metrics['Test F1']}, "
...
rf_configs = {
    ...
    "Overfit Config\n(n=1000, depth=None, leaf=1)": {
        ...
        "model": RandomForestClassifier(
            ...
            min_samples_split=2, max_features=None,  # use ALL features at each split
            class_weight="balanced", random_state=42, n_jobs=-1,
        ),
...
def make_combined_figure(lr_results, rf_results, filename):
    ...
    for row_idx, (results, model_name) in enumerate([
        ...
        for col_idx, metric in enumerate(["Test F1", "ROC-AUC", "Recall"]):
            ...
            bars = ax.barh(range(n), vals, color=colors, alpha=0.85,
...
make_combined_figure(logreg_results, rf_results,
                     "part2/model_results/hp_sensitivity_combined.png")
