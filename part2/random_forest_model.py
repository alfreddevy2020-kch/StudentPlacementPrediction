...
ax.barh(top_mdi["feature"][::-1], top_mdi["importance"][::-1], color=colors[::-1], alpha=0.85, edgecolor="white")
ax.set_xlabel("MDI Importance")
ax.set_title("Random Forest — MDI Feature Importance (Top 15)\nNote: MDI can be biased toward high-cardinality / continuous features")
...
ax.barh(top_perm["feature"], top_perm["importance"],
        xerr=top_perm["std"], color=PALETTE["primary"], alpha=0.85,
        edgecolor="white", ecolor="#555555", capsize=4)
ax.set_xlabel("Permutation Importance (mean F1 decrease ± std)")
ax.set_title("Random Forest — Permutation Importance (Top 15, on Test Set)\nMeasures actual predictive impact by shuffling each feature")
ax.set_facecolor(PALETTE["light"])
fig.patch.set_facecolor("white")
plt.tight_layout()
...
compare_imp = pd.DataFrame({
    ...
    "Permutation (normalised)": all_features_perm.clip(lower=0) / all_features_perm.clip(lower=0).sum(),
}).fillna(0)
...
width = 0.4
ax.barh(x - width/2, compare_imp["MDI (normalised)"],         width, label="MDI (biased)",         color=PALETTE["primary"],   alpha=0.85, edgecolor="white")
ax.barh(x + width/2, compare_imp["Permutation (normalised)"], width, label="Permutation (honest)", color=PALETTE["secondary"], alpha=0.85, edgecolor="white")
ax.set_yticks(x)
...
fig, ax = plt.subplots(figsize=(10, 7))
ax.barh(top_drop["feature"], top_drop["importance"], color=colors_drop, alpha=0.85, edgecolor="white")
...
ax.axvline(best_params["n_estimators"], color=PALETTE["secondary"], linestyle="--", linewidth=1.8,
...
# --- Learning Curve ---
fig, ax = plt.subplots(figsize=(9, 5))
ax.plot(train_sizes_abs, lc_train_mean, "o-", color=PALETTE["primary"],   label="Training F1", linewidth=2)
ax.plot(train_sizes_abs, lc_val_mean,   "s-", color=PALETTE["secondary"], label="CV F1 (5-fold)", linewidth=2)
