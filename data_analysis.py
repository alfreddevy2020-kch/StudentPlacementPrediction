...
mean_by_placement = df.groupby("placement_status")[
    numerical_features
].mean().round(2)
...
median_by_placement = df.groupby("placement_status")[
    numerical_features
].median().round(2)
...
certification_placement = pd.crosstab(
...
attendance_placement = pd.crosstab(
    df["attendance_group"],
    df["placement_status"],
    normalize="index"
).mul(100).round(2)
...
study_placement = pd.crosstab(
    df["study_hours_group"],
    df["placement_status"],
    normalize="index"
).mul(100).round(2)
...
df["placement_binary"] = df["placement_status"].map({
    "Not Placed": 0,
    "Placed": 1
})
...
print(
    placed_students["salary_package_lpa"].describe()
)
...
salary_by_branch = (
    placed_students
    .groupby("branch")["salary_package_lpa"]
...
salary_by_tier = (
    placed_students
    .groupby("college_tier")["salary_package_lpa"]
    .agg(["count", "mean", "median", "min", "max"])
    .round(2)
)
